...
const resolveApiUrl = () =>
  `${ENV.llmApiBaseUrl.replace(/\/$/, "")}/chat/completions`;

const resolveApiKey = () => ENV.llmApiKey;

const assertApiKey = () => {
  if (!resolveApiKey()) {
    throw new Error(
      "No LLM credentials configured. Set OPENAI_API_KEY in your environment."
    );
  }
};
...
export async function listLLMModels(): Promise<ModelsResponse> {
  ...
  const url = `${ENV.llmApiBaseUrl.replace(/\/$/, "")}/models`;
